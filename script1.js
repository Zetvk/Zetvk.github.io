...
if (result) {
  //Добавляем код для отправки формы на сервер с помощью jQuery.post
  $.post("ajax.php", // URL-адрес скрипта на сервере
  { // Данные, которые будут отправлены
    mail: inputMail.value,
    name: inputName.value,
    phone: inputPhone.value,
    address: inputAddress.value,
    submit: true // Флаг, указывающий, что форма была отправлена
  },
  function(data, status) { // Функция, которая будет вызвана при успешном ответе сервера
    // Выводим сообщение об успешной записи в модальном окне Bootstrap
    $("#modal-body").html(data); // Задаем содержимое тела модального окна
